...
func init() {
	registry.RegisterRegistry("consul", NewRegistry)
}
...
func configure(c *consulRegistry, opts ...registry.Option) {
	...
	if c.opts.Context != nil {
		...
		if co, ok := c.opts.Context.Value(configKey).(*consul.Config); ok {
			config = co
		}
		if cn, ok := c.opts.Context.Value(connectKey).(bool); ok {
			c.connect = cn
		}

		// Use the consul query options passed in the options, if available
		if qo, ok := c.opts.Context.Value(queryOptionsKey).(*consul.QueryOptions); ok && qo != nil {
			c.queryOptions = qo
		}
		if as, ok := c.opts.Context.Value(allowStaleKey).(bool); ok {
...
func (c *consulRegistry) Deregister(s *registry.Service, opts ...registry.DeregisterOption) error {
	...
	node := s.Nodes[0]
	return c.Client().Agent().ServiceDeregister(node.SrvNodeID)
}
...
func (c *consulRegistry) Register(s *registry.Service, opts ...registry.RegisterOption) error {
	...
	var regGrpcInterval time.Duration
	...
	if options.GrpcTTL > 0 {
		regGrpcCheck = true
		regGrpcInterval = options.GrpcTTL
	} else {
		if c.opts.Context != nil {
			if tcpCheckInterval, ok := c.opts.Context.Value(tcpCheckKey).(time.Duration); ok {
				regTCPCheck = true
	...
	h, err := hash.Hash(s, hash.FormatV2, nil)
	if err != nil {
		return err
	}
	...
	if ok && v == h {
		if options.TTL == time.Duration(0) {
			...
			if err := c.Client().Agent().PassTTL("service:"+node.SrvNodeID, ""); err == nil {
				return nil
			}
		}
	}
	...
	} else if regGrpcCheck {
		deregTTL := getDeregisterTTL(regGrpcInterval)
		check = &consul.AgentServiceCheck{
			GRPC:                           node.Address,
			Interval:                       fmt.Sprintf("%v", regGrpcInterval),
			DeregisterCriticalServiceAfter: fmt.Sprintf("%v", deregTTL),
		}

	} else if options.TTL > time.Duration(0) { // if the TTL is greater than 0 create an associated check
	...
	return c.Client().Agent().PassTTL("service:"+node.SrvNodeID, "")
}
